...
# -*- coding: utf-8 -*-
r"""
Orca_gcode_2_Makerbot_Z18.py — v11.5a-mac

Mac-focused robustness:
- Accepts both Orca placeholders: [output_filepath] and [tmp_output_filepath]
- Handles file:// URIs (strips scheme & unquotes)
- Waits until the input file is fully written (size+mtime stabilization, up to 10s)
- Supports .gcode.gz by auto-decompressing to a safe temporary path
- Quotes-safe: filenames with spaces or '#'
- Everything else like v11.5a: full parsing, DF-friendly meta, fuzzy-safe surfaces, adaptive RDP, thumbnails
...
import os, re, sys, time, math, json, zipfile, gzip, tempfile
...
WAIT_AFTER_WRITE_S = 1.5              # base wait (kept from v11.5a)
STABILIZE_TIMEOUT_S = 10.0            # new: max time to wait for file to finish writing
STABILIZE_POLL_S = 0.25               # polling interval
...
def _sleep_stable_base(p: Path, secs: float):
    """Legacy v11.5a small sleep before more robust stabilization."""
    try:
        t0 = p.stat().st_mtime
        time.sleep(secs)
...
def _stabilize_until_quiet(p: Path, timeout_s: float = STABILIZE_TIMEOUT_S, poll_s: float = STABILIZE_POLL_S) -> bool:
    """Wait until file exists and (size, mtime) remain unchanged across two polls."""
...
def _from_file_uri(s: str) -> str:
    """Strip file:// and unquote; return a system path string."""
    if s.lower().startswith("file://"):
        u = urlparse(s)
        # On macOS, paths are typically absolute; unquote the path portion.
        return unquote(u.path)
    return s

def _resolve_input_arg(argv) -> Optional[Path]:
    # Prefer the last non-empty, non-option token. Accept [output_filepath] or [tmp_output_filepath].
    for tok in argv[1:][::-1]:
        ...
        try:
            p = Path(t)
            # accept as-is; existence checked later after stabilization
            return p
        except Exception:
            continue
    return None

def _maybe_decompress_gz(src: Path) -> Tuple[Path, Optional[Path]]:
    """
    If src is *.gz, decompress to a temp file and return (path_to_plain_gcode, temp_path).
    If not gz, return (src, None).
    """
    name = src.name.lower()
    if name.endswith(".gcode.gz") or name.endswith(".gz"):
        tmp = Path(tempfile.gettempdir()) / (src.stem if name.endswith(".gz") else src.with_suffix("").name)
        # Ensure .gcode extension if needed
        if tmp.suffix.lower() != ".gcode":
            tmp = tmp.with_suffix(".gcode")
...
def parse_gcode(gpath: Path) -> Parsed:
    ...
    re_diam   = re.compile(r";\s*filament_?diameter(?:\[\d+\])?\s*[:=]\s*([0-9.+-]+)", re.I)
    ...
    with gpath.open("r", encoding="utf-8", errors="ignore") as fh:
        for raw in fh:
            ...
            if m and H.object_name is None:
                ...
                rawname=re.sub(r"\.(stl|obj|3mf|gcode)$","", rawname, flags=re.I)
            ...
            if is_extrude:
                ...
                # update bbox raw
                # (safe guards)
                # Note: we still track bbox later after shift; keep raw min/max here
                # but ensure variables exist
                global_xmin = locals().get('xmin')
                global_xmax = locals().get('xmax')
                global_ymin = locals().get('ymin')
                global_ymax = locals().get('ymax')
                if x_new is not None:
                    xmin_val = x_new if global_xmin is None else min(global_xmin, x_new)
                    xmax_val = x_new if global_xmax is None else max(global_xmax, x_new)
                    xmin, xmax = xmin_val, xmax_val
                if y_new is not None:
                    ymin_val = y_new if global_ymin is None else min(global_ymin, y_new)
                    ymax_val = y_new if global_ymax is None else max(global_ymax, y_new)
...
def _build_with_eps(parsed: Parsed, eps_nonsurface: float, eps_surface: float):
    ...
    if xmin is not None and xmax is not None and ymin is not None and ymax is not None:
        # If coords look like 0..305, shift to center ±152.5
...
# ---------- Main ----------
def main(argv):
    # Resolve path from CLI (works for [output_filepath] and [tmp_output_filepath])
    in_path = _resolve_input_arg(argv)

    # Fallback: latest *.gcode in watch dir (useful if user exports there)
    if in_path is None:
        fallback = _latest_gcode(WATCH_DIR)
        if not fallback:
            time.sleep(3.0)
            print("No gcode file found")
            return 2
        in_path = fallback

    # Normalize file:// URIs and make absolute
    in_path = Path(_from_file_uri(str(in_path))).expanduser().resolve()

    # Small base delay (legacy behavior), then robust stabilization
    _sleep_stable_base(in_path, WAIT_AFTER_WRITE_S)
    _stabilize_until_quiet(in_path, STABILIZE_TIMEOUT_S, STABILIZE_POLL_S)

    if not in_path.exists():
        print(f"[ERROR] G-code not found: {in_path}", file=sys.stderr)
        return 2

    # Accept gzipped G-code by auto-decompressing to temp file
    plain_path, tmp_created = _maybe_decompress_gz(in_path)
    try:
        parsed = parse_gcode(plain_path)
        ...
        base = derive_basename_from_header(plain_path) or plain_path.stem
        build_zip(in_path if in_path.exists() else plain_path, meta, toolpath, base)
        return 0
    finally:
        # Clean up temp decompressed file if we created one
        if tmp_created and tmp_created.exists():
            try: tmp_created.unlink()
            except: pass
